...
// src/routes/photo-albums.ts
var photoAlbumsRoutes = new Hono2();
...
var registrationsRoutes = new Hono2();
...
adminRoutes.put("/news/:id", async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    body.urls ? JSON.stringify(body.urls) : existing.urls,
...
adminRoutes.put("/courses/:id", async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    body.active !== void 0 ? body.active ? 1 : 0 : existing.active,
...
adminRoutes.get("/registrations", async (c) => {
  ...
  if (courseId) {
    ...
    const { results: results2 } = await c.env.DB.prepare(query + " ORDER BY r.created_at DESC").bind(courseId).all();
...
adminRoutes.get("/stats", async (c) => {
  const [news, courses, members, registrations] = await Promise.all([
    c.env.DB.prepare("SELECT COUNT(*) as count FROM news").first(),
    c.env.DB.prepare("SELECT COUNT(*) as count FROM courses").first(),
    c.env.DB.prepare("SELECT COUNT(*) as count FROM members").first(),
    c.env.DB.prepare("SELECT COUNT(*) as count FROM registrations").first()
  ]);
  return c.json({
    news: news?.count || 0,
...
// .wrangler/tmp/bundle-anhWjk/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-anhWjk/middleware-loader.entry.ts
